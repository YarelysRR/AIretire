...
def check_image_quality(image_bytes):
    ...
    try:
        ...
        if faces:
            face_area_ratio = sum(face.get("faceRectangle", {}).get("width", 0) * 
                                face.get("faceRectangle", {}).get("height", 0) 
                                for face in faces) / (1024 * 768)  # Normalized to common resolution
        ...
        if adult_content.get("isAdultContent", False) or adult_content.get("isRacyContent", False):
            quality_score *= 0.5
        ...
        return {"success": False, "error": str(e), "qualityScore": 0}