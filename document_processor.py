    ...
    try:
        ...
        if document_type == "id":
            ...
            if hasattr(result, "documents") and result.documents:
                ...
                structured_data = {
                    field_name: field.value for field_name, field in doc.fields.items()
                }

                # Ensure we have a documentNumber for our app logic
                if "DocumentNumber" in structured_data:
                    structured_data["documentNumber"] = structured_data[
                        "DocumentNumber"
                    ]
            ...
            if "documentNumber" not in structured_data:
                return {
                    "success": False,
                    "error": "Unable to extract document number",
                    "data": {},
                }
            ...
            if hasattr(result, "key_value_pairs"):
                for kv in result.key_value_pairs:
                    if (
                        kv.key
                        and kv.value
                        and hasattr(kv.key, "content")
                        and hasattr(kv.value, "content")
                    ):
            ...
            structured_data = {"key_value_pairs": key_value_pairs, "tables": tables}
        else:
